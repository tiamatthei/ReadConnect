...
for book in data:
    ...
    insert_query = """
        ...
        RETURNING id
    """
    authors = execute_values(cur, insert_query, author_values, fetch=True)
    author_ids = [row[0] for row in authors]
    conn.commit()
    ...
    execute_values(
        cur, insert_query, book_author_values)
    conn.commit()
    ...
    insert_query = """
        ...
        RETURNING id
    """
    categories =execute_values(cur, insert_query, category_values, fetch=True)
    category_ids = [row[0] for row in categories]
    conn.commit()

    # Insert book-categories relationships
    book_category_values = [(book_id, category_id)
